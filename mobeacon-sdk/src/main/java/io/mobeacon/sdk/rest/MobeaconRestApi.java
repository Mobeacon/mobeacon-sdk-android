...
public interface MobeaconRestApi {
    ...
    @GET("/app/{appKey}/geo")
    Observable<List<Location>> getNearestLocations(@Path("appKey") String appKey, @Query("lat") double latitude, @Query("lon") double longitude, @Query("rad") float radius, @Query("n") int limit);
}
